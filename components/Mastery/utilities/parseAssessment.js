function parseAssessment(mArr){
    ...
    Object.keys(AssessmentArrS).forEach(
      (LOID,id)=>{
        AssessmentArrS[LOID] = {
          mcountN:0,
          mcountA:0,
    ...
    studentRows.forEach(
      (row,id) => {
        if(
          (
            !!classNoF && row.classNo.indexOf(classNoF) < 0
          ) 
          ||
          (
            !!studentName
            &&
            (row.title + ' ' + row.lastName).toLowerCase()
              .indexOf(studentName.toLowerCase()) < 0
          )
          ||
          (
            false
        ...
        if(!rowsByStu.hasOwnProperty(row.uEntryID)){
          rowsByStu[row.uEntryID] = {
            ratings:Object.assign({},alignModel),
            studentRowsId:id,
            recentrating:''
          }
        Object.keys(rowsByStu[row.uEntryID].ratings).forEach(
          (key,id)=>{
